...
**Ficcionari** és una PWA multijugador del *joc del diccionari* per jugar en persona amb
...
7. `reveal` — resultats: es revela la real, l'autoria, vots i punts; auto-avança en 20s
   (`RevealPhase`).
...
- Auto-avanç del reveal: 20s. Barra festiva d'auto-avanç per als jugadors **fixa a baix,
  arran de vora** (`flush`: gruixuda, cantonades rectes, però el progrés que avança té el cap
  dret arrodonit); es renderitza a `GamePage` fora del contenidor animat perquè el `fixed`
  s'ancori de veritat (un ancestre amb `transform` trencava el `fixed`). Per al narrador, la
  barra va **dins del botó** "Ronda següent".
- Últim jugador que queda per escriure (amb 4+ jugadors): compte enrere de **30s** amb barra
  sota el quadre; en expirar auto-envia el text si n'hi ha.
...
- **Nova opció de partida**: columna a `games` (migració idempotent) → `database.types.ts` →
  ...
  → `GameOptionsValue` + control a `GameOptionsFields.tsx` → consumir-la on toqui → claus i18n
  ca/es → avisar l'usuari d'executar la migració.
